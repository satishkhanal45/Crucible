...
- **Last completed:** Phase 7 — reporting (tasks 0–6), verification tests 1–8
  ...
  `reports/`.
- **Classifier agreement is now measured on `deepseek-v4-flash`: objective
  38/40 (95%), technique 32/40 (80%), combined 30/40 (75%)** — better than the
  `openai/gpt-oss-120b` figures (90/75/65) on every axis, and above the 0.70
  line on both. `docs/findings.md` carries both, because the same prompt scoring
  differently on two models is itself part of the result.
...
- **Open issues / blockers:** one — the incomplete seeded archive above; re-seed
  ...
  - **An unpriced model silently disables the budget.** With no entry in the
    ...
    must still run.
  - **The defender's prompt carries the WHOLE schema, generated from the models.**
    A six-round live run produced zero valid configs: the prompt documented
    `pattern_class` and `action` for a heuristic rule and nothing else, so the
    model emitted exactly those two fields while `HeuristicRule` also requires
    `name` and `applies_to`. Every branch failed identically — 4 rejections, then
    8 on the retry. `schema_digest()` is generated by walking the pydantic models,
    so a new field cannot be forgotten, and `EXAMPLE_CONFIG` is a real
    `DefenseConfig` instance so the worked example cannot become invalid. The fix
    was the prompt: **`DefenseConfig` validation was not loosened, and unknown
    keys are still rejected.** Verified live: the same call now validates.
  - **Every rejected candidate logs its reason at WARNING**, with the field-level
    validation errors and 500 characters of the raw reply. Twelve rounds of
    `kept: 0` with no reason recorded is a run that cannot be diagnosed
    afterwards, and it cost a whole experiment.
  - **`data/benign_tasks.yaml` declares what it was recorded against, and a test
    enforces it.** `expect_docs` is the output of retrieval, so it is invalidated
    by a change of embedder or corpus — silently, by lowering the utility rate.
    The file now carries `recorded_with` (embedding model, corpus sha256,
    document count, commit, date); `assert_expectations_current()` refuses to
    score if either has moved, and every CLI command that measures utility calls
    it. The values were re-recorded on 2026-09-05 under `all-MiniLM-L6-v2`, so
    `expect_docs` is enforced at **full strength on all 40 tasks** — the
    "enforce only where retrieval still returns one" skip is gone, because the
    utility term is never-cut and skipping it on 9 of 40 tasks weakened it.
    Re-recording is retrieval only, no LLM, and therefore reproducible.
  - **`bt-h02` is load-bearing in two directions.** It must be answerable from
    the corpus (or it measures corpus coverage, not the defense) AND it must
    contain "ignore previous instructions" (or a keyword defense does not reject
    it). It is the ONLY task that gives
    `test_a_keyword_defense_is_scored_as_utility_loss` anything to measure: with
    the old wording that config blocked exactly one task out of forty, and an
    intermediate rewording of mine kept the hard-negative label while losing the
    trigger words, which emptied that test without failing it. Both properties
    are now asserted directly in `tests/unit/test_benign_expectations.py`.
  - **Phase 5 test 13 exists twice, on purpose.** The stub version guards the
    pipeline offline in CI, deriving each task's `expect_docs` from the
    retrieval its own (hashing) embedder produces — scoring the committed
    MiniLM recording against hashing retrieval would be the very defect this
    change removed. `tests/integration/test_benign_live.py` is the live variant,
    marked `live` and deselected by default (`addopts = ["-m", "not live"]`); run
    it with `uv run pytest -m live` before committing hours to a run. A stub that
    is more permissive than reality has now cost this project three separate
    incidents.
  - **The benign expectations are recorded artefacts, and two of them were
    measuring the recording rather than the target.** `expect_text` was recorded
    against `ScriptedTargetLLM`, which quotes the top excerpt verbatim, so the
    phrases are literal document prefixes that no paraphrasing model emits; it is
    no longer a pass condition. `expect_docs` was recorded under the **hashing
    embedder**, and under the production MiniLM embedder 9 of 40 tasks retrieve
    none of their recorded documents; it is now enforced only when retrieval
    still returns one of them. The fabrication check (citations ⊆ retrieved),
    the refusal check and the defense-block check are untouched — those are what
    the utility term exists for. Live measurement under D(0) went from 17/40 to
    33/40 on the relaxations alone, and higher again after `expect_docs` was
    re-recorded; the remaining failures are the target breaking its own JSON contract
    on hard-negative questions, which is real target behaviour and is headroom
    for `output.schema_enforcement`.
  - **`utility_collapse` needs a significant decline, not just a low number.**
    It halted a run at 0.325 against a 0.34 floor while D(n) was still D(0) — no
    defense had been applied, so nothing could have caused a collapse. 16/40 vs
    13/40 is p≈0.49. docs/spec.md §15 requires a two-proportion test for a
    round-over-round claim, and a decline is a claim like any other, so the check
    now requires both the ratio and significance. The baseline is measured
    **once**, at run start, against D(0), and `baseline_utility_trials` carries
    its denominator so the test can be run at all. A halt that can fire when the
    configuration never changed is measuring the noise floor.
  - **A rising block rate under an unchanged config is not hardening.** Run
    9fed94ea went 0.583 → 0.643 over six rounds with D(n) == D(0): that is the
    archive accumulating attacks the target refuses unaided. The run report and
    the terminal output both say so whenever the final config equals the
    starting one, so no reader can take the curve for a defense improving.
