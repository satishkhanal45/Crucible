...
- **Phase status:** Phases 0–7 complete; Phase 8 not started
- **Effort spent:** 11.5 / 13.0 units
...
- **Next up:** Phase 8 — the experiments and `docs/findings.md`. Every figure in
  findings is regenerated by script from stored rounds and attempts; nothing is
  hand-typed. Run `crucible archive reclassify --dry-run` first: it gates the
  phase, because coverage is only as trustworthy as the seed labels.
...
| 8 | not started | | Preceded by a hotfix: provider error mapping, model pricing, model ids in config, free-tier pacing, CLI error boundary. 759 tests green. |
